...
def convertCoordination():
    ...
    for _img,_txt in zip(img_dir, txt_dir):
        ...
        coordinate = []
        #dh dw : 이미지의 가로, 세로 크기
        img = cv2.imread(_img,) #cv2.IMREAD_COLOR)
...
location = convertCoordination() # getcoord()
print(location)
print(type(location))
print(type(location[1][0]))
print(location[1][0])
...
print(trim_image)
print(type(trim_image))

# contour = 

img2 = cv2.imread('./img/00110011001.jpg', cv2.IMREAD_COLOR)
imgx = cv2.rectangle(img2,(89, 1026), (53, 1319), (0,255,0), 3 )  # (1026, 89), (1319, 53)
cv2.imshow('contours', imgx)
cv2.waitKey()
cv2.destroyAllWindows()

# image = cv2.drawContours(img2, location[1][0], -1, (0,255,0), 3)
